...
- **TPU quantization (MaxText-aligned)**: Full-model INT8/FP8 via Qwix/AQT, including MoE expert ragged matmuls—see section *Quantization on TPU (MaxText-aligned, including MoE)* in [`Integrating_MaxText_MoE_Strategies.md`](Integrating_MaxText_MoE_Strategies.md). Kappa’s functional stack has no in-tree Qwix hook; that doc contrasts MaxText practice with this repo.
- **Chunked prefill (Qwen3)**: `forward_prefill_chunk` in `kappa/qwen3/transformer.py` (Gemma3-style dense-prefix KV). `generate(..., prefill_chunk_size=N)` and `scripts/infer_qwen3.py --prefill-chunk-size N` when `N>0` and prompt length `> N` (requires uniform valid length per row or batch size 1). JIT: `kappa/qwen3/jit_inference.jit_forward_prefill_chunk`. Sanity: `scripts/verify_qwen3_prefill_chunk.py`.
