...
prompt_jax_jit_caching = sanitize_text_for_tokenizer(_jax_jit_caching)
prompt_jax_static_args = sanitize_text_for_tokenizer(_jax_static_args)
prompt_jax_jit_compilation = sanitize_text_for_tokenizer(_jax_jit_compilation)
prompt_marketing_campaign = sanitize_text_for_tokenizer(_marketing_campaign)
prompt_bitter_lesson = sanitize_text_for_tokenizer(_bitter_lesson)

_PROMPT_LIBRARY = {
    "jax_jit_caching": prompt_jax_jit_caching,
    "jax_static_args": prompt_jax_static_args,
    "jax_jit_compilation": prompt_jax_jit_compilation,
    "marketing_campaign": prompt_marketing_campaign,
    "bitter_lesson": prompt_bitter_lesson,
}


def list_long_prompt_names():
    """Stable names for long prompt templates."""
    return sorted(_PROMPT_LIBRARY)


def get_long_prompt(name):
    """Get a long prompt template by name."""
    if name not in _PROMPT_LIBRARY:
        raise KeyError(f"Unknown prompt name {name!r}. Choices: {list_long_prompt_names()}")
    return _PROMPT_LIBRARY[name]
